...
# We need to update docs an cms platform.
# This variable should be set to `replica`
DATABASE_CONNECTION_REPLICA_NAME = "default"

DATABASES = {
    DATABASE_CONNECTION_DEFAULT_NAME: dj_database_url.config(
        default="postgres://cms:cms@localhost:5432/cms", conn_max_age=600
    ),
    # TODO: We need to add read only user to cms platfrom, and we need to update
    # docs.
    # DATABASE_CONNECTION_REPLICA_NAME: dj_database_url.config(
    #     default="postgres://cms_read_only:cms@localhost:5432/cms",
    #     conn_max_age=600,
    # ),
}
...
STATICFILES_DIRS = [
    ("images", os.path.join(PROJECT_ROOT, "cms", "static", "images"))
]
STATICFILES_FINDERS = [
...
context_processors = [
    ...
    "cms.site.context_processors.site",
]
...
JWT_MANAGER_PATH = os.environ.get(
    "JWT_MANAGER_PATH", "cms.core.jwt_manager.JWTManager"
)

MIDDLEWARE = [
    "django.middleware.security.SecurityMiddleware",
    "django.middleware.common.CommonMiddleware",
    "cms.core.middleware.request_time",
    # "cms.core.middleware.discounts",
    "cms.core.middleware.google_analytics",
    "cms.core.middleware.site",
    "cms.core.middleware.plugins",
    "cms.core.middleware.jwt_refresh_token_middleware",
]
...
if ENABLE_DEBUG_TOOLBAR:
    ...
    try:
        ...
        MIDDLEWARE.append("cms.graphql.middleware.DebugToolbarMiddleware")
...
LOGGING = {
    ...
    "formatters": {
        ...
        "json": {
            "()": "cms.core.logging.JsonFormatter",
        ...
        "celery_json": {
            "()": "cms.core.logging.JsonCeleryFormatter",
            "datefmt": "%Y-%m-%dT%H:%M:%SZ",
            "format": (
        ...
        "celery_task_json": {
            "()": "cms.core.logging.JsonCeleryTaskFormatter",
            "datefmt": "%Y-%m-%dT%H:%M:%SZ",
            "format": (
                "%(asctime)s %(levelname)s %(celeryTaskId)s %(celeryTaskName)s "
                "%(message)s "
    ...
    "loggers": {
        ...
        "cms": {"level": "DEBUG", "propagate": True},
        "cms.graphql.errors.handled": {
            "handlers": ["default"],
            "level": "INFO",
            "propagate": False,
        },
...
TEST_RUNNER = "cms.tests.runner.PytestTestRunner"
...
if AWS_MEDIA_BUCKET_NAME:
    DEFAULT_FILE_STORAGE = "cms.core.storages.S3MediaStorage"
elif GS_MEDIA_BUCKET_NAME:
    DEFAULT_FILE_STORAGE = "cms.core.storages.GCSMediaStorage"
elif AZURE_CONTAINER:
    DEFAULT_FILE_STORAGE = "cms.core.storages.AzureMediaStorage"
...
AUTHENTICATION_BACKENDS = [
    "cms.core.auth_backend.JSONWebTokenBackend",
    "cms.core.auth_backend.PluginBackend",
]
...
CELERY_TASK_ROUTES = {
    "cms.plugins.webhook.tasks.observability_reporter_task": {
        "queue": "observability"
    },
    "cms.plugins.webhook.tasks.observability_send_events": {
        "queue": "observability"
    },
}
...
CELERY_BEAT_SCHEDULE = {
    "delete-empty-allocations": {
        "task": "cms.warehouse.tasks.delete_empty_allocations_task",
        "schedule": timedelta(days=1),
    },
    "deactivate-preorder-for-variants": {
        "task": "cms.product.tasks.deactivate_preorder_for_variants_task",
        "schedule": timedelta(hours=1),
    },
    "delete-expired-reservations": {
        "task": "cms.warehouse.tasks.delete_expired_reservations_task",
        "schedule": timedelta(days=1),
    },
    "delete-expired-checkouts": {
        "task": "cms.checkout.tasks.delete_expired_checkouts",
        "schedule": crontab(hour=0, minute=0),
    },
    "delete-outdated-event-data": {
        "task": "cms.core.tasks.delete_event_payloads_task",
        "schedule": timedelta(days=1),
    },
    "deactivate-expired-gift-cards": {
        "task": "cms.giftcard.tasks.deactivate_expired_cards_task",
        "schedule": crontab(hour=0, minute=0),
    },
    "update-stocks-quantity-allocated": {
        "task": "cms.warehouse.tasks.update_stocks_quantity_allocated_task",
        "schedule": crontab(hour=0, minute=0),
    },
    "delete-old-export-files": {
        "task": "cms.csv.tasks.delete_old_export_files",
        "schedule": crontab(hour=1, minute=0),
    },
    "send-sale-toggle-notifications": {
        "task": "cms.discount.tasks.send_sale_toggle_notifications",
        "schedule": initiated_sale_webhook_schedule,
    },
    "update-products-search-vectors": {
        "task": "cms.product.tasks.update_products_search_vector_task",
...
if OBSERVABILITY_ACTIVE:
    CELERY_BEAT_SCHEDULE["observability-reporter"] = {
        "task": "cms.plugins.webhook.tasks.observability_reporter_task",
...
BUILTIN_PLUGINS = [
    ...
    "cms.plugins.openid_connect.plugin.OpenIDConnectPlugin",
]

# Plugin discovery
EXTERNAL_PLUGINS = []
installed_plugins = pkg_resources.iter_entry_points("cms.plugins")
...
#
# Refer to our guide on https://docs.cms.io/docs/next/guides/opentracing-jaeger/.
#
