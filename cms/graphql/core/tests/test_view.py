...
from .... import __version__ as cms_version
...
def test_invalid_query_graphql_errors_are_logged_in_another_logger(
    ...
    assert graphql_log_handler.messages == [
        "cms.graphql.errors.handled[INFO].GraphQLError"
    ]
...
def test_invalid_syntax_graphql_errors_are_logged_in_another_logger(
    ...
    assert response.status_code == 400
    assert graphql_log_handler.messages == [
        "cms.graphql.errors.handled[INFO].GraphQLSyntaxError"
    ]
...
def test_permission_denied_query_graphql_errors_are_logged_in_another_logger(
    ...
    assert response.status_code == 200
    assert graphql_log_handler.messages == [
        "cms.graphql.errors.handled[INFO].PermissionDenied"
    ]
...
@mock.patch("cms.graphql.product.schema.resolve_collection_by_id")
def test_unexpected_exceptions_are_logged_in_their_own_logger(
    ...
    assert response.status_code == 200
    assert graphql_log_handler.messages == [
        "cms.graphql.errors.unhandled[ERROR].NotImplementedError"
    ]
...
@mock.patch("cms.graphql.views.cache.set")
@mock.patch("cms.graphql.views.cache.get")
...
def test_generate_cache_key_use_cms_version():
    cache_key = generate_cache_key(INTROSPECTION_QUERY)
    assert cms_version in cache_key
