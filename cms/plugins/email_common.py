...
from ..post.post_images import get_post_image_placeholder
...
def get_post_image_thumbnail(this, size, image):
    ...
    media = PostMedia.objects.filter(image__name=image.name).first()
    if not media:
        return get_post_image_placeholder(size)
    return prepare_image_proxy_url(media.id, "PostMedia", size, None)
...
def send_email(
    ...
    helpers = {
        ...
        "get_post_image_thumbnail": get_post_image_thumbnail,
        "compare": compare,
    }
