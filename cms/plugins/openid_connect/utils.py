...
def get_user_from_oauth_access_token_in_jwt_format(
    ...
    if use_scope_permissions and audience_in_token:
        permissions = get_cms_permissions_qs_from_scope(scope)
        if not permissions and token_permissions:
            permissions = get_cms_permissions_from_list(token_permissions)
...
def get_cms_permissions_qs_from_scope(scope: str) -> QuerySet[Permission]:
    scope_list = scope.lower().strip().split()
    return get_cms_permissions_from_list(scope_list)


def get_cms_permissions_from_list(permissions: list) -> QuerySet[Permission]:
    cms_permissions_str = [s for s in permissions if s.startswith("cms:")]
    if SALEOR_STAFF_PERMISSION in cms_permissions_str:
        cms_permissions_str.remove(SALEOR_STAFF_PERMISSION)
    if not cms_permissions_str:
        return Permission.objects.none()

    permission_codenames = list(
        map(lambda perm: perm.replace("cms:", ""), cms_permissions_str)
    )
    permissions = get_permissions_from_codenames(permission_codenames)
    return permissions


def get_cms_permission_names(permissions: QuerySet) -> List[str]:
