...
from ..utils import (
    ...
    get_cms_permission_names,
    get_cms_permissions_qs_from_scope,
    get_user_from_token,
    get_user_info,
...
def test_fetch_jwks_raises_error(monkeypatch, error):
    ...
    monkeypatch.setattr("cms.plugins.openid_connect.utils.requests.get", mocked_get)
...
@pytest.mark.vcr
@mock.patch("cms.plugins.openid_connect.utils.cache.set")
def test_fetch_jwks(mocked_cache_set):
    jwks_url = "https://cms.io/.well-known/jwks.json"
...
def test_get_or_create_user_from_token_missing_email(id_payload):
    ...
    with pytest.raises(AuthenticationError):
        get_or_create_user_from_payload(id_payload, "https://cms.io/oauth")
...
def test_create_tokens_from_oauth_payload(monkeypatch, id_token, id_payload):
    ...
    user = get_or_create_user_from_payload(id_payload, "https://cms.io/oauth")
    permissions = get_cms_permissions_qs_from_scope(auth_payload.get("scope"))
    perms = get_cms_permission_names(permissions)
...
def test_get_cms_permissions_from_scope():
    auth_payload = {
        "access_token": "FeHkE_QbuU3cYy1a1eQUrCE5jRcUnBK3",
        "refresh_token": "refresh",
        "scope": (
            "openid profile email offline_access cms:manage_orders "
            "cms:non_existing cms cms: cms:manage_users"
        ),
    }
    expected_permissions = {"MANAGE_USERS", "MANAGE_ORDERS"}
    permissions = get_cms_permissions_qs_from_scope(auth_payload.get("scope"))
    permission_names = get_cms_permission_names(permissions)
...
def test_get_user_info_raises_decode_error(monkeypatch):
    ...
    monkeypatch.setattr(
        "cms.plugins.openid_connect.utils.requests.get", Mock(return_value=response)
    )

    user_info = get_user_info("https://cms.io/userinfo", "access_token")
