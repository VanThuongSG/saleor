...
def test_external_refresh_from_cookie(
    ...
    monkeypatch.setattr(
        "cms.plugins.openid_connect.utils.get_decoded_token",
    ...
    monkeypatch.setattr(
        "cms.plugins.openid_connect.plugin.OAuth2Session.refresh_token",
    ...
    cms_refresh_token = create_jwt_refresh_token(
        admin_user, oauth_refresh_token, csrf_token, plugin.PLUGIN_ID
    )
    request = rf.request()
    request.COOKIES[JWT_REFRESH_TOKEN_COOKIE_NAME] = cms_refresh_token
...
def test_external_refresh_from_input(
    ...
    request = rf.request()
    data = {"refreshToken": cms_refresh_token}
    response = plugin.external_refresh(data, request, None)
...
def test_external_refresh_with_scope_permissions(
    ...
    monkeypatch.setattr(
        "cms.plugins.openid_connect.plugin.get_parsed_id_token",
        Mock(return_value=mocked_jwt_validator),
    )
    oauth_payload = {
        "access_token": "FeHkE_QbuU3cYy1a1eQUrCE5jRcUnBK3",
        ...
        "id_token": id_token,
        "scope": (
            "openid profile email offline_access cms:manage_orders cms:staff"
        ),
        "expires_in": 86400,
        "token_type": "Bearer",
        "expires_at": 1600851112,
...
def test_external_obtain_access_tokens(
    ...
    monkeypatch.setattr(
        "cms.plugins.openid_connect.plugin.OAuth2Session.fetch_token",
        mocked_fetch_token,
    )
    ...
    user = get_or_create_user_from_payload(claims, oauth_url="https://cms.io/oauth")
...
def test_external_obtain_access_tokens_with_permissions(
    ...
    oauth_payload = {
        ...
        "refresh_token": "refresh",
        "id_token": id_token,
        "scope": "openid profile email offline_access cms:manage_orders",
        "expires_in": 86400,
        "token_type": "Bearer",
        "expires_at": 1600851112,
    }
    ...
    user = get_or_create_user_from_payload(claims, "https://cms.io/oauth")
    user.is_staff = True
    expected_tokens = create_tokens_from_oauth_payload(
        oauth_payload,
        user,
...
@freeze_time("2019-03-18 12:00:00")
@pytest.mark.vcr
def test_external_obtain_access_tokens_with_cms_staff(
    openid_plugin, monkeypatch, rf, id_token, id_payload
):
    mocked_jwt_validator = MagicMock()
    mocked_jwt_validator.__getitem__.side_effect = id_payload.__getitem__
    ...
    oauth_payload = {
        ...
        "refresh_token": "refresh",
        "id_token": id_token,
        "scope": "openid profile email offline_access cms:staff",
        "expires_in": 86400,
        "token_type": "Bearer",
        "expires_at": 1600851112,
    }
...
def test_validate_plugin_configuration(plugin_configuration, openid_plugin):
    configuration = plugin_configuration(
        ...
        client_secret="c_secret",
        enable_refresh_token=True,
        oauth_authorization_url="http://cms.io/auth",
        oauth_token_url="http://cms.io/token",
        json_web_key_set_url="http://cms.io/jwks",
    )
    conf = PluginConfiguration(active=True, configuration=configuration)
    plugin = openid_plugin()
    plugin.validate_plugin_configuration(conf)
...
def test_authenticate_user(openid_plugin, id_payload, customer_user, monkeypatch, rf):
    ...
    monkeypatch.setattr(
        "cms.plugins.openid_connect.plugin.get_token_from_request", lambda _: token
    )
...
def test_authenticate_user_with_access_token(
    ...
    plugin = openid_plugin(
        ...
        json_web_key_set_url="https://cms.io/.well-known/jwks.json",
        user_info_url="https://cms.io/userinfo",
    ...
    monkeypatch.setattr(
        "cms.plugins.openid_connect.plugin.get_token_from_request",
        lambda _: "OAuth_access_token",
    )

    # decode access token returns payload when access token is in JWT format
    monkeypatch.setattr(
        "cms.plugins.openid_connect.utils.decode_access_token", lambda x, y: None
    )
    # mock request to api to fetch user info details
    monkeypatch.setattr(
        "cms.plugins.openid_connect.utils.get_user_info",
        lambda x, z: user_info_response,
    )
    # mock cache used for caching user info details
    monkeypatch.setattr("cms.plugins.openid_connect.utils.cache.set", Mock())
...
def test_authenticate_user_with_access_token_unable_to_fetch_user_info(
    ...
    # mock request to api to fetch user info details
    monkeypatch.setattr(
        "cms.plugins.openid_connect.utils.get_user_info", lambda x, z: None
    )
    # mock cache used for caching user info details
    monkeypatch.setattr("cms.plugins.openid_connect.utils.cache.set", Mock())
...
def test_authenticate_user_with_jwt_access_token(
    ...
    # decode access token returns payload when access token is in JWT format
    monkeypatch.setattr(
        "cms.plugins.openid_connect.utils.decode_access_token",
...
def test_authenticate_staff_user_with_jwt_access_token_and_staff_scope(
    ...
    decoded_access_token["scope"] = "openid profile email cms:staff"
    decoded_token = MagicMock()
    decoded_token.__getitem__.side_effect = decoded_access_token.__getitem__
    decoded_token.get.side_effect = decoded_access_token.get
...
def test_authenticate_staff_user_with_jwt_access_token_and_staff_in_permissions_field(
    ...
    decoded_access_token["permissions"] = ["cms:staff"]
    decoded_token = MagicMock()
    decoded_token.__getitem__.side_effect = decoded_access_token.__getitem__
    decoded_token.get.side_effect = decoded_access_token.get
...
def test_authenticate_staff_user_with_jwt_access_token_with_permissions_field(
    ...
    decoded_access_token["scope"] = "openid profile email"
    decoded_access_token["permissions"] = ["cms:manage_orders", "cms:manage_apps"]
    decoded_token = MagicMock()
    decoded_token.__getitem__.side_effect = decoded_access_token.__getitem__
    decoded_token.get.side_effect = decoded_access_token.get
...
def test_authenticate_user_with_jwt_access_token_unable_to_fetch_user_info(
    ...
    # mock request to api to fetch user info details
    monkeypatch.setattr(
        "cms.plugins.openid_connect.utils.get_user_info", Mock(return_value=None)
    )
    # mock cache used for caching user info details
    monkeypatch.setattr("cms.plugins.openid_connect.utils.cache.set", Mock())
...
def test_authenticate_user_unable_to_decode_token(openid_plugin, monkeypatch, rf):
    plugin = openid_plugin(user_info_url="")
    monkeypatch.setattr(
        "cms.plugins.openid_connect.plugin.get_token_from_request", lambda _: "ABC"
    )
    user = plugin.authenticate_user(rf.request(), None)
    assert user is None
