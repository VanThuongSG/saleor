...
def test_get_plugins_manager(settings):
    plugin_path = "cms.plugins.tests.sample_plugins.PluginSample"
...
def test_manager_with_default_configuration_for_channel_plugins(
    ...
    settings.PLUGINS = [
        "cms.plugins.tests.sample_plugins.ChannelPluginSample",
        "cms.plugins.tests.sample_plugins.PluginSample",
    ]
    manager = get_plugins_manager()
...
def test_manager_get_plugins_with_channel_slug(
    ...
    settings.PLUGINS = [
        "cms.plugins.tests.sample_plugins.PluginInactive",
        "cms.plugins.tests.sample_plugins.PluginSample",
    ]
    manager = get_plugins_manager()
...
@pytest.mark.parametrize(
    ...
    [(["cms.plugins.tests.sample_plugins.PluginSample"], "1.0"), ([], "15.0")],
)
...
@pytest.mark.parametrize(
    ...
    [(["cms.plugins.tests.sample_plugins.PluginSample"], "1.0"), ([], "0.0")],
)
...
@pytest.mark.parametrize(
    "plugins, shipping_amount",
    [(["cms.plugins.tests.sample_plugins.PluginSample"], "1.0"), ([], "10.0")],
)
...
def test_manager_get_checkout_line_tax_rate_sample_plugin(
    ...
    plugins = ["cms.plugins.tests.sample_plugins.PluginSample"]
...
@pytest.mark.parametrize(
    "plugins, amount",
    [(["cms.plugins.tests.sample_plugins.PluginSample"], "1.0"), ([], "12.30")],
)
...
@pytest.mark.parametrize(
    "plugins, show_taxes",
    [(["cms.plugins.tests.sample_plugins.PluginSample"], True), ([], False)],
)
...
@pytest.mark.parametrize(
    ...
    [
        ([], sample_none_data),
        (["cms.plugins.tests.sample_plugins.PluginSample"], sample_tax_data),
    ],
)
...
@pytest.mark.parametrize(
    "plugins, amount",
    [(["cms.plugins.tests.sample_plugins.PluginSample"], "15.0"), ([], "0")],
)
...
def test_plugin_add_new_configuration(
    ...
    manager = PluginsManager(
        plugins=["cms.plugins.tests.sample_plugins.PluginInactive"]
    )
...
def test_manager_serve_list_of_payment_gateways(channel_USD):
    ...
    plugins = [
        "cms.plugins.tests.sample_plugins.PluginSample",
        "cms.plugins.tests.sample_plugins.ActivePaymentGateway",
        "cms.plugins.tests.sample_plugins.InactivePaymentGateway",
    ]
    manager = PluginsManager(plugins=plugins)
...
def test_manager_serve_list_all_payment_gateways_specified_currency(channel_USD):
    ...
    plugins = [
        "cms.plugins.tests.sample_plugins.ActivePaymentGateway",
        "cms.plugins.tests.sample_plugins.InactivePaymentGateway",
        "cms.plugins.tests.sample_plugins.ActiveDummyPaymentGateway",
    ]
    manager = PluginsManager(plugins=plugins)
    assert (
...
@mock.patch(
    "cms.plugins.manager.PluginsManager._PluginsManager__run_method_on_single_plugin"
)
...
def test_run_check_payment_balance(channel_USD):
    plugins = ["cms.plugins.tests.sample_plugins.ActiveDummyPaymentGateway"]
...
def test_run_check_payment_balance_not_implemented(channel_USD):
    plugins = ["cms.plugins.tests.sample_plugins.ActivePlugin"]
...
@mock.patch("cms.plugins.manager.base_calculations.base_checkout_total")
