...
def patch_get_buffer(buffer):
    with patch(
        "cms.webhook.observability.utils.get_buffer", return_value=buffer
    ) as get_buffer:
        yield get_buffer
...
def patch_get_webhooks(observability_webhook_data):
    with patch(
        "cms.webhook.observability.utils.get_webhooks",
...
@patch("cms.webhook.observability.utils.cache.get")
@patch("cms.webhook.observability.utils.get_webhooks_for_event")
...
@patch("cms.webhook.observability.utils.ApiCall.report")
...
@patch("cms.webhook.observability.utils.put_event")
